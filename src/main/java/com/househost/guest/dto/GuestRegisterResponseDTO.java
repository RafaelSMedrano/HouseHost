...
public class GuestRegisterResponseDTO {
    ...
    public GuestRegisterResponseDTO(Guest guest) {
        this.id = guest.getId();
        ...
        this.bookingIds = guest.getBookings().stream().map(booking -> booking.getId()).toList();
        this.stayIds = guest.getStays().stream().map(stay -> stay.getId()).toList();
