...
class FinancialTransactionValidationService {
    ...
    void validateRequest(FinancialTransactionRequestDTO request) {
        ...
        if (request.senderType == null) {
            throw new FinanceException("Tipo do pagante e obrigatorio.");
        }
        if (request.senderId == null) {
            throw new FinanceException("Identificador do pagante da transacao financeira e obrigatorio.");
        }
        if (request.receiverType == null) {
            throw new FinanceException("Tipo do recebedor e obrigatorio.");
        }
    ...
    void validateSource(FinancialTransactionSourceType sourceType, Long sourceId) {
        if (sourceType != null && sourceId == null) {
            throw new FinanceException(
                    "Identificador da origem da transacao e obrigatorio quando sourceType e informado."
            );
        }
        if (sourceType == FinancialTransactionSourceType.PLAN
                || sourceType == FinancialTransactionSourceType.INSTALLMENT) {
            throw new FinanceException(
                    "Origem de propriedade financeira so pode ser atribuida pelo fluxo interno."
            );
        }
    }

    void validateStandaloneCreationType(FinancialTransactionType type) {
        if (type != FinancialTransactionType.STANDARD) {
            throw new FinanceException(
                    "Tipo de plano financeiro so pode ser atribuido pelo fluxo correspondente."
            );
        }
    }
    ...
    void validateImmutableUpdateFields(FinancialTransaction transaction, FinancialTransactionRequestDTO request) {
        if (transaction.getSenderType() != request.senderType
                || !transaction.getSenderId().equals(request.senderId)
                || transaction.getReceiverType() != request.receiverType
                || !transaction.getReceiverId().equals(request.receiverId)) {
        ...
        if (transaction.getAmount().compareTo(request.amount) != 0) {
            throw new FinanceException("Valor da transacao nao pode ser alterado.");
        }
        if (transaction.getType() != request.type) {
            throw new FinanceException("Tipo da transacao nao pode ser alterado.");
        }
        if (request.status != null && request.status != transaction.getStatus()) {
            throw new FinanceException("Status da transacao deve ser alterado pelo fluxo de liquidacao.");
        }
    }
}
