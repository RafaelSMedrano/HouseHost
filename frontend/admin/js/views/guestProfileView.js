import { deleteFinancialTransaction, findAllBookings, findAllFinancialTransactions, findGuestById, revealGuestContact, settleFinancialTransaction } from "../api.js?v=2026-06-15-contact-reveal";
...
function renderProfile(container, guest, bookings, transactions, options) {
    ...
    const status = activeBookingsInStay.length > 0 ? "IN_STAY" : activeBookings.length > 0 ? "IN_BOOKING" : normalizeGuestStatus(guest.status);
