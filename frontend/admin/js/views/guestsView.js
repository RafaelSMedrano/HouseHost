import { deleteGuest, findAllBookings, findAllGuests, findGuestsByCity, findGuestsByEmail, findGuestsByName, findGuestsByPhone, findMetricsSummary } from "../api.js?v=2026-05-18-metrics";
...
function bindGuestsView(container, options) {
    const state = { guests: [], allGuests: [], bookings: [], query: "", type: "", financialStatus: "", status: "", remoteSearch: false, searchRequestId: 0 };
    ...
    search.addEventListener("input", async () => {
        ...
        const searchLength = state.query.replace(/\s/g, "").length;
        const requestId = ++state.searchRequestId;

        if (!state.query) {
            state.guests = state.allGuests;
        ...
        try {
            const responses = state.query.includes("@")
                ? [await findGuestsByEmail(state.query)]
                : /^\D*\d[\d\D]*$/.test(state.query)
                    ? [await findGuestsByPhone(state.query)]
                    : await Promise.all([findGuestsByName(state.query), findGuestsByCity(state.query)]);
            if (requestId !== state.searchRequestId) {
                return;
            }
            const guestIds = new Set(responses.flatMap((response) => response.data || []).map((guest) => Number(guest.id)));
            state.guests = state.allGuests.filter((guest) => guestIds.has(Number(guest.id)));
            state.remoteSearch = true;
            renderGuestsTable(container, state, options);
        } catch (error) {
            if (requestId === state.searchRequestId) {
...
async function loadGuests(container, state, options) {
    try {
        const [response, bookingsResponse, metricsResponse] = await Promise.all([
            findAllGuests(),
            findAllBookings(),
            findMetricsSummary(),
        ]);
        ...
        state.guests = (response.data || []).map((guest) => ({
            ...guest,
            displayStatus: resolveGuestDisplayStatus(guest, state.bookings),
        }));
        state.allGuests = state.guests;
...
function renderGuestsTable(container, state, options) {
    ...
    tbody.innerHTML = data.map((guest) => `
      <tr>
        <td><div class="guest-row-main"><span class="guest-row-avatar">${initialsFor(guest.fullName)}</span><div><button class="guest-name-button" type="button" data-open-guest="${guest.id}">${escapeHtml(guest.fullName || "-")}</button><small>${guestTypeText(guest.guestType)} · ${escapeHtml(guest.email || "E-mail não informado")}</small></div></div></td>
...
function getFilteredGuests(state) {
    return state.guests.filter((guest) => {
        ...
        const matchesQuery = state.remoteSearch || !state.query || text.includes(state.query);
...
async function handleDeleteGuest(container, state, options, guestId) {
    ...
    try {
        ...
        state.allGuests = state.allGuests.filter((guest) => guest.id !== guestId);
...
function resolveGuestDisplayStatus(guest, bookings) {
    if (hasBookingWithStatus(guest.id, bookings, "IN_STAY")) {
        return "IN_STAY";
    }
    ...
    if (hasBookingWithStatus(guest.id, bookings, "FINISHED")) {
        return "GOT_CHECKOUT";
    }
...
function hasBookingWithStatus(guestId, bookings, status) {
    return bookings.some((booking) => Number(booking.guestId) === Number(guestId)
        && normalizeBookingStatus(booking.status) === status);
}
...
function hasBooking(guestId, bookings) {
    return bookings.some((booking) => Number(booking.guestId) === Number(guestId)
        && ["UNCONFIRMED", "CONFIRMED"].includes(normalizeBookingStatus(booking.status)));
}
