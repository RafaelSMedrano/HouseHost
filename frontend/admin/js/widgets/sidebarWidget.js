...
import { canAccessView } from "../permissions.js?v=2026-07-26-processing-governance";
...
<div class="sidebar">
  ...
  <nav>
    ...
    ${hasAnyView(role, ["checkin", "checkout", "reservations", "guests"]) ? '<div class="nav-group">Hospedagem</div>' : ""}
...
function navItem(role, view, icon, label, active = false, suffix = "") {
    ...
    return `<button class="nav-item ${active ? "active" : ""}" type="button" data-view="${view}"><i class="ti ${icon}"></i> ${label} ${suffix}</button>`;
}
