import { deleteBooking, findAllBookings, findMetricsSummary } from "../api.js?v=2026-08-11-api-log-transport";
...
const statusLabels = {
    ...
    got_checkout: "Check-out realizado",
    cancelada: "Cancelada",
};
    ...
    <div class="reservations-toolbar-row">
      ...
      <div class="reservations-toolbar">
        ...
        <select id="reservation-status" class="reservations-select">
          ...
          <option value="got_checkout">Check-out realizado</option>
...
function renderTable(container, state, data, options) {
    ...
    tbody.innerHTML = data.map((reservation) => `
      <tr class="reservation-click-row" data-reservation-id="${escapeHtml(reservation.rawId)}">
        ...
        <td><div class="td-actions">${renderReservationActions(reservation, options)}</div></td>
      </tr>
    `).join("");
    ...
    tbody.querySelectorAll("[data-checkout-reservation]").forEach((button) => {
        button.addEventListener("click", () => {
            if (typeof options.onCreateCheckOut === "function") {
                options.onCreateCheckOut(Number(button.dataset.checkoutReservation));
            }
        });
    });
...
function renderReservationActions(reservation, options) {
    const checkAction = reservation.status === "got_checkin"
        ? `<button class="reservation-checkin-btn" type="button" data-checkout-reservation="${escapeHtml(reservation.rawId)}" title="Check-out"><i class="ti ti-logout"></i><span>Check-out</span></button>`
        : `<button class="reservation-checkin-btn" type="button" data-checkin-reservation="${escapeHtml(reservation.rawId)}" title="Check-in"><i class="ti ti-login"></i><span>Check-in</span></button>`;

    const deleteAction = options.canDelete
        ? `<button type="button" data-delete-reservation="${escapeHtml(reservation.rawId)}" title="Excluir reserva"><i class="ti ti-trash"></i></button>`
        : "";

    return `${checkAction}<button type="button" data-edit-reservation="${escapeHtml(reservation.rawId)}" title="Editar reserva"><i class="ti ti-pencil"></i></button>${deleteAction}`;
}
