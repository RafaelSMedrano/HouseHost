import { changeSupplierStatus, findSupplierById, reviewSupplierRelationship } from "../api.js?v=2026-08-11-api-log-transport";
...
async function loadProfile(container, options) {
    try {
        ...
        container.querySelector(".supplier-feedback").textContent = error.message || "Não foi possível carregar o fornecedor.";
        container.querySelector("[data-supplier-profile]").innerHTML = '<button class="dashboard-back-btn" type="button" data-back aria-label="Voltar">Voltar</button>';
...
function renderProfile(container, supplier, options) {
    ...
    container.querySelector("[data-supplier-profile]").innerHTML = `
      <header class="supplier-header supplier-profile-header"><div><button class="dashboard-back-btn" type="button" data-back aria-label="Voltar">Voltar</button><span>Fornecedor #${escapeHtml(supplier.id)}</span><h1>${escapeHtml(supplier.tradeName || supplier.officialName)}</h1><p>${escapeHtml(supplier.officialName)}</p></div><div class="supplier-profile-actions"><button class="dashboard-back-btn" type="button" data-edit><i class="ti ti-pencil"></i>Editar</button><button class="dashboard-back-btn" type="button" data-status>${supplier.status === "ACTIVE" ? "Desativar" : "Ativar"}</button></div></header>
