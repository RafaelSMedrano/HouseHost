import { deleteGuest, findAllBookings, findAllGuests, findGuestsByCity, findGuestsByEmail, findGuestsByName, findGuestsByPhone, findMetricsSummary } from "../api.js?v=2026-08-11-api-log-transport";
  ...
  <div class="content guests-page">
    ...
    <div class="guests-toolbar">
      ...
      <select id="guests-status" class="guests-select">
        ...
        <option value="WITH_UNCONFIRMED_BOOKING">Com reserva não confirmada</option>
        <option value="WITH_CONFIRMED_BOOKING">Com reserva confirmada</option>
...
function renderGuestsTable(container, state, options) {
    ...
    tbody.innerHTML = data.map((guest) => `
      <tr>
        ...
        <td>${guestStatusBadge(guest.status)}</td>
...
function getFilteredGuests(state) {
    return state.guests.filter((guest) => {
        ...
        const matchesStatus = !state.status || normalizeGuestStatus(guest.status) === state.status;
