import { findAllBookings, findAllRooms } from "../api.js?v=2026-08-11-api-log-transport";
    ...
    shell.innerHTML = `
      <div class="timeline-grid" style="--timeline-width:${width}px; --timeline-cell:${scale.cellWidth}px; --timeline-days:${scale.days};">
        ...
        <div class="timeline-body">
          ${state.rooms.map((room) => renderRoomRow(room, state.bookings, state.startDate, visibleEnd, scale)).join("")}
        </div>
      </div>
    `;
...
function renderRoomRow(room, bookings, startDate, visibleEnd, scale) {
    const roomBookings = bookings
        ...
        .filter((booking) => normalizeStatus(booking.status) !== "CANCELED");
    const status = currentRoomStatus(room, bookings);
...
function normalizeStatus(status) {
    return String(status || "UNCONFIRMED").toUpperCase();
}
...
function currentRoomStatus(room, bookings) {
...
function isBlockingBooking(booking) {
    return ["UNCONFIRMED", "CONFIRMED", "IN_STAY"].includes(normalizeStatus(booking.status));
}
