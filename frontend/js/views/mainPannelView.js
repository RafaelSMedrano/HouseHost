import { findAllBookings, findAllCheckIns, findAllCheckOuts, findAllStays, findMetricsSummary } from "../api.js?v=2026-05-18-metrics";
...
async function loadDashboardData(container, options) {
    try {
        const [bookingsResponse, staysResponse, checkInsResponse, checkOutsResponse, metricsResponse] = await Promise.all([
            findAllBookings(),
            findAllStays(),
            findAllCheckIns(),
            findAllCheckOuts(),
        ...
        renderTodayEvents(container, { bookings, stays, checkIns, checkOuts }, options);
    } catch (error) {
...
function renderTodayEvents(container, { bookings, stays, checkIns, checkOuts }, options) {
    const today = currentDate();
    const doneCheckInBookingIds = new Set(checkIns.filter((item) => isSameDate(item.actualCheckInAt, today)).map((item) => Number(item.bookingId)));
    const doneCheckOutStayIds = new Set(checkOuts.filter((item) => isSameDate(item.actualCheckOutAt, today)).map((item) => Number(item.stayId)));
    ...
    const checkOutRows = stays
        .filter((stay) => stay.expectedCheckOutDate === today && normalizeKey(stay.status) === "ACTIVE" && !doneCheckOutStayIds.has(Number(stay.id)))
        .map((stay) => renderTodayItem(stay.guestName, "Check-out", stay.roomNumber, "Até hoje", "Checkout", "act-out", "checkout", stay.id));
    ...
    container.querySelectorAll("[data-dashboard-checkout-stay]").forEach((button) => {
        button.addEventListener("click", () => options.onCreateCheckOut?.(Number(button.dataset.dashboardCheckoutStay)));
    });
}
...
function renderTodayItem(name, info, roomNumber, time, action, actionClass, operation, id) {
    const actionAttribute = operation === "checkin"
        ...
        : `data-dashboard-checkout-stay="${escapeHtml(id)}"`;
...
function isBlockingBooking(booking) {
    return ["PENDING", "CONFIRMED", "GOT_CHECKIN"].includes(normalizeKey(booking.status));
}
