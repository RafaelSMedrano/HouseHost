...
function renderCheckIns(container, state) {
    const data = filteredItems(state, checkInSearchText).sort((a, b) => String(b.actualCheckInAt || "").localeCompare(String(a.actualCheckInAt || "")));
    ...
    tbody.innerHTML = data.map((item) => `
      <tr>
        ...
        <td>${formatDateTime(item.actualCheckInAt)}</td>
...
function renderCheckOuts(container, state) {
    ...
    tbody.innerHTML = data.map((item) => `
      <tr>
        <td><div class="guest-row-main"><span class="guest-row-avatar">${initialsFor(item.guestName)}</span><div><strong>${escapeHtml(item.guestName || "-")}</strong><small>Estadia #${escapeHtml(item.stayId || "-")}</small></div></div></td>
        <td>${escapeHtml(item.roomNumber || "-")}</td>
...
function checkOutSearchText(item) {
    return `${item.guestName || ""} ${item.roomNumber || ""} ${item.stayId || ""} ${item.status || ""}`.toLowerCase();
}
