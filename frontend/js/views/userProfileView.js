import { updateUserPhoto, updateUserProfile } from "../api.js?v=2026-05-20-profile-crud";
...
export function renderUserProfileView(containerId, user = {}, options = {}) {
  ...
  <div class="content user-profile-page">
    <div class="up-top-actions">
      <button class="dashboard-back-btn user-profile-back" type="button"><i class="ti ti-arrow-left"></i> Voltar</button>
      ...
      <section class="up-section-card">
        ...
        <div class="up-section-body">
          <div class="up-form-grid">
            ...
            ${selectField("Cargo / Função", "up-role", role, ["Chefe Executivo Organizacional (CEO)", "Diretor Geral de Tecnologia (CTO)", "Administrador", "Gerente", "Recepção", "Governança"], "Apenas administradores devem alterar cargos.")}
          </div>
        </div>
      </section>
      ...
      <section class="up-section-card">
        ...
        <div class="up-section-body">
          <div class="up-perm-list">
            ${permissionRow("Reservas", "Criar, editar, cancelar e visualizar todas as reservas")}
            ${permissionRow("Hóspedes", "Cadastrar, editar e excluir registros de hóspedes")}
            ${permissionRow("Quartos", "Gerenciar quartos, tarifas e disponibilidade")}
            ${permissionRow("Financeiro", "Visualizar e registrar pagamentos e relatórios")}
            ${permissionRow("Usuários do sistema", "Criar e gerenciar outros usuários e cargos")}
          </div>
        </div>
      </section>
...
function selectField(label, id, value, options, hint = "") {
    return `
      <label class="up-field">
        <span class="up-field-label">${escapeHtml(label)}</span>
        <div class="up-input-wrap no-icon"><select id="${escapeHtml(id)}">${options.map((option) => `<option ${option === value ? "selected" : ""}>${escapeHtml(option)}</option>`).join("")}</select></div>
        ${hint ? `<small>${escapeHtml(hint)}</small>` : ""}
      </label>
    `;
}
...
function permissionRow(name, desc) {
    return `<div class="up-perm-item"><div><strong>${escapeHtml(name)}</strong><span>${escapeHtml(desc)}</span></div><em>Acesso total</em></div>`;
}
